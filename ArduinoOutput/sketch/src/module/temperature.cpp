    ...
    while (wait_for_heatup) {
      ...
      if (temp_meas_ready) { // temp sample ready
        ...
        #endif
        {
          ...
          char buffer [50];
          int n;
          n = sprintf (buffer, "temp is %f", temp_hotend[0].celsius);
          SERIAL_ECHOLN(buffer);
          if (temp_hotend[0].celsius >= EXTRUDER_AUTO_FAN_TEMPERATURE
          #if EXTRUDERS > 1
              || temp_hotend[1].celsius > EXTRUDER_AUTO_FAN_TEMPERATURE
          #endif
          #if EXTRUDERS > 2
              || temp_hotend[2].celsius > EXTRUDER_AUTO_FAN_TEMPERATURE
          #endif
              )
          {
              PORTJ |= _BV(6);
