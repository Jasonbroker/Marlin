...
void abortPrint(bool bQuickstop)
{
    ...
    if (primed)
    {
        ...
        plan_set_e_position((end_of_print_retraction / volume_to_filament_length[active_extruder]) - (fwretract.retracted ? fwretract.settings.retract_length : 0), active_extruder, true);
...
static void lcd_sd_menu_filename_callback(uint8_t nr, uint8_t offsetY, uint8_t flags)
{
    ...
    if (nr == 0)
    {
        ...
        if (buffer[0] == '\0')
        {
            ...
            LCD_CACHE_TYPE(idx) = card.flag.filenameIsDir ? 1 : 0;
...
static void lcd_menu_print_error_sd()
{
    ...
    int_to_string(card.getSd2Card().errorCode(), buffer+5);
