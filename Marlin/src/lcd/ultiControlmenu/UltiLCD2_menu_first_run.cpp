...
#include "../../Marlin.h"
#include "../../sd/cardreader.h"//This code uses the card.longFilename as buffer to store data, to save memory.
...
static void lcd_menu_first_run_material_load_heatup()
{
    ...
    int16_t target = thermalManager.degTargetHotend(0) - 10 - 20;
    if (temp < 0) temp = 0;
    if (temp > target)
    {
        for(uint8_t e=0; e<EXTRUDERS; ++e)
